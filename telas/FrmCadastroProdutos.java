...
public class FrmCadastroProdutos extends javax.swing.JFrame {
    ...
    public FrmCadastroProdutos() {
        ...
        // CAMINHO PARA SALVAR NO "BANCO DE DADOS" NO ARQUIVO DADOS_PRODUTOS.txt
        this.strCaminhoArquivo = "C:\\Users\\ACER\\OneDrive\\Documentos\\NetBeansProjects\\Trabalho1_Desktop\\CadastroLoja\\src\\main\\java\\DADOS.TXT\\DADOS_PRODUTOS.txt";
    ...
    private void initComponents() {
        ...
        tblProduto.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Código", "Nome", "Preço", "Qntd em Estoque", "Categoria", "Fornecedor", "Data de Validade"
            }
        ) {
...
private boolean validarCampos() { // VALIDAÇÃO DOS CAMPOS NA TELA DE CADASTRO DE PRODUTOS.
    if (txtCodigo.getText().trim().isEmpty()) {
        mostrarErro("Código do Produto", txtCodigo);
        return false;
    }
...
private void gravarDados() { // BOTÃO SALVAR.
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(strCaminhoArquivo, true))) {
        // Adiciona o produto no arquivo sem sobrescrever os dados anteriores
        writer.newLine();
        writer.write("====================");
        writer.newLine();
        writer.write("Código: " + txtCodigo.getText());
        writer.newLine();
        writer.write("Nome: " + txtNome.getText());
        writer.newLine();
        writer.write("Preço: " + txtPreco.getText());
        writer.newLine();
        writer.write("Quantidade em Estoque: " + txtQntdEstoque.getText());
        writer.newLine();
        writer.write("Categoria: " + txtCategoria.getText());
        writer.newLine();
        writer.write("Fornecedor: " + txtFornecedor.getText());
        writer.newLine();
        writer.write("Data de Validade: " + txtDataValidade.getText());        
        JOptionPane.showMessageDialog(null, "Cadastro salvo com sucesso", "Mensagem do sistema!", JOptionPane.INFORMATION_MESSAGE);
...
private void limpar (){ // BOTÃO LIMPAR.
    // LIMPA OS DADOS DIGITADOS NA TELA DE CADASTRO DE PRODUTOS, É ACIONADO NO BOTÃO DE LIMPAR.
    try{
        txtCodigo.setText("");
        txtNome.setText("");
        txtPreco.setText("");
...
private void editar(String caminhoArquivo) throws IOException { // BOTÃO EDITAR.
        // BOTÃO EDITAR NA TELA DE CADASTRO PRODUTO.
         List<String> linhas = new ArrayList<>();
        String nomeEditado = txtCodigo.getText();

        try (BufferedReader br = new BufferedReader(new FileReader(caminhoArquivo))) {
            String linha;
            boolean encontrado = false;
            while ((linha = br.readLine()) != null) {
                if (linha.startsWith("Código:") && linha.contains(nomeEditado)) {
                    encontrado = true;
                    linhas.add("Código: " + txtCodigo.getText());  
                    linhas.add("Nome: " + txtNome.getText());
                    linhas.add("Preço: " + txtPreco.getText());
                    linhas.add("Quantidade em Estoque: " + txtQntdEstoque.getText());
                    linhas.add("Categoria: " + txtCategoria.getText());
                    linhas.add("Fornecedor: " + txtFornecedor.getText());
                    linhas.add("Data de Validade: " + txtDataValidade.getText());
                    linhas.add("------------------------------------------------------");
                    while ((linha = br.readLine()) != null && !linha.startsWith("Código:")) {
                    }
                }
                if (linha != null) {
                    linhas.add(linha);
...
private void excluir(String caminhoArquivo, String nomeExcluir) throws IOException { // BOTÃO EXCLUIR.
        // BOTÃO EXCLUIR TUDO NA TELA DE CADASTRO DE PRODUTO.
        List<String> linhas = new ArrayList<>();
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(caminhoArquivo))) {
            String linha;
            while ((linha = br.readLine()) != null) {
                if (linha.startsWith("Código:") && linha.contains(nomeExcluir)) {
                    excluir = true;
                }
                if (excluir) {
                    if (linha.startsWith("------------------------------------------------------")) {
...
private void configurarTabela() { 
    try {
        int totalColunas = tblProduto.getColumnModel().getColumnCount();
        ...
        SPProdutos.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
        SPProdutos.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        SPProdutos.setViewportView(tblProduto); 

        // Atualizar o JScrollPane
        SPProdutos.revalidate();
        SPProdutos.repaint();
...
private void carregarDados(String caminhoArquivo) {
    ...
    try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(caminhoArquivo), StandardCharsets.UTF_8))) {
        String linha;
        String[] DADOS = new String[7];  // Array para armazenar os dados de um produto
        
        while ((linha = br.readLine()) != null) {
            linha = linha.trim();  // Remove espaços extras

            // Ignora linhas vazias e separadores de produtos
            if (!linha.isEmpty() && !linha.startsWith("-")) {
                ...
                if (chaveValor.length == 2) {
                    ...
                    switch (chave) {
                        case "Código" -> DADOS[0] = valor;
                        case "Nome" -> DADOS[1] = valor;
                        case "Preço" -> DADOS[2] = valor;
                        case "Quantidade em Estoque" -> DADOS[3] = valor;
                        case "Categoria" -> DADOS[4] = valor;
                        case "Fornecedor" -> DADOS[5] = valor;
                        case "Data de Validade" -> DADOS[6] = valor;
                    }
                }
            }

            // Quando todos os dados de um produto estão preenchidos, adiciona à tabela
            if (DADOS[0] != null && DADOS[1] != null && DADOS[2] != null && DADOS[3] != null && DADOS[4] != null && DADOS[5] != null && DADOS[6] != null) {
                model.addRow(DADOS);  // Adiciona os dados do produto na tabela
                DADOS = new String[7];  // Reseta os dados para o próximo produto
            }
        }
        
        // Adiciona a última linha se não foi adicionada no loop (caso o último produto esteja completo)
        if (DADOS[0] != null && DADOS[1] != null && DADOS[2] != null && DADOS[3] != null && DADOS[4] != null && DADOS[5] != null && DADOS[6] != null) {
            model.addRow(DADOS);
        }

    } catch (IOException e) {
        JOptionPane.showMessageDialog(null, "Erro ao carregar os dados do arquivo!", "Erro", JOptionPane.ERROR_MESSAGE);
    }
}
...
private void carregarDadosNosCampos(int row) {
        try {
            txtCodigo.setText((String) tblProduto.getValueAt(row, 0));
            txtNome.setText((String) tblProduto.getValueAt(row, 1));
            txtPreco.setText((String) tblProduto.getValueAt(row, 2));
            txtQntdEstoque.setText((String) tblProduto.getValueAt(row, 3));
            txtCategoria.setText((String) tblProduto.getValueAt(row, 4));
            txtFornecedor.setText((String) tblProduto.getValueAt(row, 5));
            txtDataValidade.setText((String) tblProduto.getValueAt(row, 6));
    ...
    private void btExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btExcluirActionPerformed
        ...
        // BOTÃO DE EXCLUIR TODO O REGISTRO NA TELA DE CADASTRO DE PRODUTO.
            String nomeExcluir = txtCodigo.getText();
    if (nomeExcluir.isEmpty()) {
        ...
        try {
            excluir(strCaminhoArquivo, nomeExcluir);  // Chama o método de exclusão
            carregarDados(strCaminhoArquivo);         // Recarrega os dados na tabela
